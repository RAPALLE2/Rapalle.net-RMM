...
export function renderClientMetric(target, client, panel) {
  ...
  if (kind === "gauge") {
    ...
    // Wert + Label - kein Überlappen, volle Boxausnutzung.
    ...
    target.innerHTML = `<div style="width:${NAT_W}px;height:${NAT_H}px;display:flex;align-items:center;gap:10px">
      <div style="position:relative;width:${gw}px;height:${gh}px;flex:none">
        ...
        <div style="position:absolute;left:0;right:0;bottom:2px;text-align:center;pointer-events:none">
          <div style="font-size:21px;font-weight:800;line-height:1">${Math.round(p)}%</div>
        </div>
      </div>
  ...
  if (kind === "donut") {
    ...
    const size = 88, sw = 13, r = (size - sw) / 2 - 1, cx = size / 2, cy = size / 2;
    ...
    target.innerHTML = `<div style="width:${NAT_W}px;height:${NAT_H}px;display:flex;align-items:center;gap:14px">
      <div style="position:relative;width:${size}px;height:${size}px;flex:none">
        ...
        <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center">
          <div style="font-size:22px;font-weight:800">${Math.round(frac * 100)}%</div>
        </div>
      </div>
      <div style="flex:1;min-width:0">
  ...
  if (kind === "line" || kind === "area" || kind === "spark") {
    ...
    head.innerHTML = spark
      ? `<span style="font-size:28px;font-weight:800;line-height:1.1">${esc(formatClientValue(preset, v))}</span>
      ...
      : `<span style="font-size:13.5px;color:var(--subtext);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(preset.label)}</span>
         <span style="font-size:20px;font-weight:800">${esc(formatClientValue(preset, v))}</span>`;
    wrap.appendChild(head);
    wrap.appendChild(timeSeriesChart(
      [{ label: panel.title || preset.label, color: "var(--accent)", values: data, timestamps: ts }],
      { width: NAT_W, height: spark ? 54 : 62, mode: kind,
  ...
  if (kind === "progress") {
    ...
    target.innerHTML = `<div style="width:${NAT_W}px;height:${NAT_H}px;display:flex;flex-direction:column;justify-content:center">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;gap:8px">
        ...
        <span style="font-size:25px;font-weight:800">${esc(formatClientValue(preset, v))}</span>
      </div>
      ...
      <div style="font-size:12.5px;color:var(--subtext);margin-top:6px;text-align:right">${Math.round(pct)}% von ${esc(formatClientValue(preset, max))}</div></div>`;
    scaleToContainer(holder); return;
  }
  ...
  if (kind === "stat") {
    ...
    wrap.innerHTML = `
      ...
      <div style="text-align:right;flex:1;min-width:0">
        <div style="font-size:15.5px;font-weight:700;color:${trendColor};margin-bottom:2px">${flat ? "→" : (up ? "▲" : "▼")} ${esc(formatClientValue(preset, Math.abs(diff)))}</div>
    ...
    wrap.querySelector(".stat-spark").appendChild(timeSeriesChart(
      ...
      { width: 112, height: 44, mode: "spark", formatValue: (x) => formatClientValue(preset, x) },
    ));
    target.appendChild(wrap);
    scaleToContainer(holder); return;
  }
  ...
  if (kind === "columns") {
    ...
    const grid = [0, 0.5, 1].map((fr) => {
      ...
      return `<line x1="${padL}" y1="${y.toFixed(1)}" x2="${W - padR}" y2="${y.toFixed(1)}" stroke="var(--border)" stroke-width="1" opacity="0.5"/>
        <text x="${padL - 4}" y="${(y + 3).toFixed(1)}" text-anchor="end" font-size="11" fill="var(--subtext)">${shortV(niceMax * fr)}</text>`;
    }).join("");
    const bars = rows.map((r, i) => {
      ...
      return `<g class="ccol" data-i="${i}" style="cursor:pointer">
        ...
        <text x="${(x + bw / 2).toFixed(1)}" y="${(y - 3).toFixed(1)}" text-anchor="middle" font-size="13" font-weight="700" fill="var(--text)">${esc(String(r.raw != null ? r.raw : Math.round(r.value || 0)))}</text>
  ...
  if (kind === "bars") {
    // Horizontale Balken: nutzt die ganze 1x1-Fläche aus, mit größeren Texten
    // und breiteren Tracks. Der Rest bleibt per "+N weitere" erreichbar.
    const all = preset.rows ? preset.rows(client) : [];
    const rows = all.slice(0, 6), hidden = all.slice(6);
    const max = Math.max(1, ...rows.map((r) => r.value || 0), 100);
    const wrap = document.createElement("div");
    wrap.className = "widget-bars";
    wrap.style.cssText = `width:${NAT_W}px;height:${NAT_H}px;display:flex;flex-direction:column;justify-content:center;gap:${rows.length <= 4 ? 7 : 4}px`;
    const fs = rows.length <= 3 ? 16.5 : rows.length <= 5 ? 15 : 14;
    const th = rows.length <= 3 ? 16 : rows.length <= 5 ? 14 : 12;
    wrap.innerHTML = rows.map((r, i) => `
      <div class="wbar-row" data-i="${i}" style="width:100%;display:flex;align-items:center;gap:8px;padding:0 4px;font-size:${fs}px;box-sizing:border-box">
        <span class="wbar-label" style="flex:0 0 66px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--subtext)">${esc(r.label)}</span>
        <span class="wbar-track" style="flex:1;min-width:0;height:${th}px;border-radius:${Math.ceil(th / 2)}px;background:var(--panel-2);overflow:hidden"><span class="wbar-fill" style="display:block;height:100%;width:${(((r.value || 0) / max) * 100).toFixed(1)}%;background:var(--accent);border-radius:${Math.ceil(th / 2)}px"></span></span>
        <span class="wbar-val" style="flex:0 0 38px;text-align:right;font-weight:700;font-variant-numeric:tabular-nums">${esc(r.raw != null ? r.raw : String(Math.round(r.value || 0)))}</span>
  ...
  if (preset.rows) {
    ...
    // Volle Breite/Höhe nutzen, besonders wenn Info-Widgets in Ordnern liegen.
    // Keine feste NAT_W-Breite mehr: der Text darf den kompletten Ordner nach rechts nutzen.
    const wrap = document.createElement("div");
    wrap.dataset.stretch = "fill";
    wrap.className = "status-info widget-info-fill";
    wrap.style.cssText = "width:100%;height:100%;display:flex;flex-direction:column;justify-content:center;gap:2px;box-sizing:border-box;padding:2px 10px;min-width:0;min-height:0;overflow:hidden";
    wrap.innerHTML = rows.map((r, i) => `
      <div data-i="${i}" style="display:grid;grid-template-columns:130px minmax(0,1fr);gap:12px;align-items:center;min-width:0"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--subtext)">${esc(r.label)}</span><b style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:left">${esc(r.raw != null ? r.raw : String(r.value ?? "—"))}</b></div>`).join("");
    const applyInfoSize = () => {
      if (!wrap.isConnected) { ro.disconnect(); return; }
      const n = Math.max(1, rows.length || 1);
      const h = Math.max(32, wrap.clientHeight || NAT_H);
      const w = Math.max(180, wrap.clientWidth || NAT_W);
      const gap = Math.max(1, Math.min(7, Math.floor(h / (n * 10))));
      const fs = Math.max(11, Math.min(20, Math.floor((h - gap * (n - 1) - 4) / n * 0.62), Math.floor(w / 45)));
      const labelW = Math.max(70, Math.min(190, Math.floor(w * 0.18)));
      wrap.style.gap = gap + "px";
      wrap.querySelectorAll("[data-i]").forEach((el) => {
        el.style.fontSize = fs + "px";
        el.style.gridTemplateColumns = `${labelW}px minmax(0, 1fr)`;
      });
    };
    const ro = new ResizeObserver(applyInfoSize);
    ro.observe(wrap);
    requestAnimationFrame(applyInfoSize);
    target.appendChild(wrap);
